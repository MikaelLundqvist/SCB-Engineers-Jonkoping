...
## Women with higher paid jobs migrate to regions with higher salaries, women with lower paid jobs migrate to regions with lower salaries, Year 2004 - 2013
Average basic salary, monthly salary and women´s salary as a percentage of men´s salary by sector, occupational group (SSYK) , sex and age. Year 2004 - 2013
Monthly salary
All sectors
...
```{r ssyk14-18_15, echo = TRUE, fig.cap = 'Women with higher paid jobs migrate to regions with higher salaries, women with lower paid jobs migrate to regions with lower salaries, positive x = tendency for women within SSYK to migrate to regions with higher salaries, positive y = SSYK with higher salaries than average for all SSYK'}
tb <- readfile("AM0110E2.csv") %>%
    rowwise() %>%
    mutate(age2 = unlist(lapply(strsplit(substr(age, 1, 5), "-"), strtoi))[1]) %>% 
    rowwise() %>%
...
salary_table <- tidy (summary (model)) %>%
  mutate (ssyk = gsub("`occupational group \\(SSYK\\)`", "", term)) 
...
merge(summary_table, sumtab, by = "ssyk", all = TRUE) %>%
  ...
  filter (statistic.y > 35)  %>%  # Filter noice with low relevance
...
merge(summary_table, sumtab, by = "ssyk", all = TRUE) %>% 
  ...
  filter (term == "log(salary.x)") %>%
  filter (statistic.y > 35)  %>%  # Filter noice with low relevance
  select (ssyk, estimate.x, estimate.y) %>%
  arrange (desc (estimate.x)) %>%
  knitr::kable( 
  booktabs = TRUE,
  caption = 'Women with higher paid jobs migrate to regions with higher salaries, women with lower paid jobs migrate to regions with lower salaries')   
```


## Women with higher paid jobs migrate to regions with higher salaries, women with lower paid jobs migrate to regions with lower salaries, Year 2014 - 2018
Average basic salary, monthly salary and women´s salary as a percentage of men´s salary by sector, occupational group (SSYK 2012) , sex and age. Year 2014 - 2018
Monthly salary
All sectors
...
```{r ssyk14-18_16, echo = TRUE, fig.cap = 'Women with higher paid jobs migrate to regions with higher salaries, women with lower paid jobs migrate to regions with lower salaries, positive x = tendency for women within SSYK to migrate to regions with higher salaries, positive y = SSYK with higher salaries than average for all SSYK'}
tb <- readfile("000000D2_1.csv") %>%
    rowwise() %>%
    mutate(age2 = unlist(lapply(strsplit(substr(age, 1, 5), "-"), strtoi))[1]) %>%  
    rowwise() %>%
...
## The correlation between salary level and the F-value for age, Year 2004 - 2013
Average basic salary, monthly salary and women´s salary as a percentage of men´s salary by sector, occupational group (SSYK) , sex and age. Year 2004 - 2013
Monthly salary
All sectors

```{r ssyk14-18_17, echo = TRUE, fig.cap = 'The correlation between salary level and the F-value for age'}
tb <- readfile("AM0110E2.csv") %>%
    rowwise() %>%
    mutate(age2 = unlist(lapply(strsplit(substr(age, 1, 5), "-"), strtoi))[1]) %>% 
    rowwise() %>%
...
sumtab %>%
  filter (term.y == "poly(age4, 3)") %>%
  filter (term == "year2") %>% 
  select (ssyk, estimate.y, statistic.y) %>%
  arrange (desc (statistic.y)) %>%
  knitr::kable( 
  booktabs = TRUE,
  caption = 'The correlation between salary level and the F-value for age')    
```

## The correlation between salary level and the F-value for age, Year 2014 - 2018
Average basic salary, monthly salary and women´s salary as a percentage of men´s salary by sector, occupational group (SSYK 2012) , sex and age. Year 2014 - 2018
Monthly salary
All sectors

```{r ssyk14-18_18, echo = TRUE, fig.cap = 'The correlation between salary level and the F-value for age'}
tb <- readfile("000000D2_1.csv") %>%
    rowwise() %>%
    mutate(age2 = unlist(lapply(strsplit(substr(age, 1, 5), "-"), strtoi))[1]) %>% 
    rowwise() %>%
